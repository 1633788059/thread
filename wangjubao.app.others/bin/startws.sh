...
if [ $PRODUCTION = "run" ]; then
	...
	JAVA_MEM_OPTS=" -server -Xms512m -Xmx1024m -XX:MaxPermSize=128m -XX:SurvivorRatio=2 -XX:+UseParallelGC "
	JAVA_OPTS=" $JAVA_MEM_OPTS "
...
echo -e "Starting havana monitor server $HOST_NAME ...\c"
	...
	STDOUT_LOG=$LOG_DIR/report.stdout.log
	nohup $JAVA_HOME/bin/java $JAVA_OPTS -classpath $CONFIG_DIR:$LIB_JARS com.wangjubao.app.others.report.main.OtherMain >> $STDOUT_LOG 2>&1 &
